...
/**
 * Texture loading, caching, and tinting subsystem for Minecraft asset rendering.
 * <p>
...
public final class TextureContext implements AutoCloseable {
    ...
    /**
     * Constructs a new {@code TextureContext} from an assets directory, overlay roots, and
    ...
    /**
     * Constructs a new {@code TextureContext} with the given data root and overlays.
     *
     ...
     * @param assetNamespaces the asset namespace registry for multi-source resolution
     */
    public TextureContext(@NotNull String dataRoot, @Nullable String embeddedTextureFile,
    ...
    public @NotNull BufferedImage getTintedTexture(@NotNull String textureId, int tintArgb, float strengthMultiplier, float blend) {
        ...
        return cache.computeIfAbsent(finalCacheKey, key -> {
            ...
            float clampedBlend = Math.clamp(blend, 0f, 1f);
            ...
            for (int y = 0; y < tinted.getHeight(); y++) {
                for (int x = 0; x < tinted.getWidth(); x++) {
                    ...
                    tintedR = Math.clamp(tintedR, 0f, 1f);
                    tintedG = Math.clamp(tintedG, 0f, 1f);
                    tintedB = Math.clamp(tintedB, 0f, 1f);
                    tintedA = Math.clamp(tintedA, 0f, 1f);
    ...
    public void registerTexture(@NotNull String textureId, @NotNull BufferedImage image, boolean overwrite) {
        if (StringUtil.isEmpty(textureId))
    ...
    private static @NotNull List<String> extractOverlayPaths(@NotNull List<OverlayRoot> overlayRoots) {
    ...
    private static @NotNull BufferedImage createMissingTexture() {
    ...
    private int @Nullable [] tryLoadTrimPaletteColors() {
    ...
    private static final class AnimationOverrideScope implements AutoCloseable {
        ...
        private final TextureContext owner;
    ...
    @Nullable
    private static BufferedImage loadImageSafe(@NotNull Path path) {
        try {
    ...
    private static @NotNull String formatFloat(float value) {
